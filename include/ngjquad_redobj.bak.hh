...
static unsigned long countw = 0;
static unsigned long countxw = 0;
...
struct optData
{
  ...
  void init()
  {
    ...
    if (this->dim == 2)
    {
      ...
      this->ipiv = (lapack_int*) calloc(N, sizeof(lapack_int));
...
struct ngjQuad
{
  ...
  static inline void optieqX (  unsigned int m, double* result, unsigned int n, 
                                const double* Zk, double* grad, 
                                void* _data )
  {
    optData* data = (optData*) _data;
  ...
  inline void init ()
  {
    ...
    double* Z0 = optdata->Z0; double* S = optdata->S; 
    if (this->dim == 2)
    {
