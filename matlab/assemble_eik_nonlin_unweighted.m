...
% Precompute Jacobians and detJ for all triangles
detJ_vec = zeros(nTri, 1);
for iTri = 1:nTri
    Pts_Ti = meshP(:,meshT(iTri,:));
    J = IncidenceMatrix(Pts_Ti);
    detJ_vec(iTri) = det(J);
end
% Reshape global coefficients
cu = reshape(cu_glb(1:M*nTri), [M, nTri]);                  % [M × nTri]
...
if nargout == 2
    H_glb = zeros(M*nTri);
    CU = reshape(cu_glb(1:M*nTri), [M, nTri]);
...
%      J = IncidenceMatrix(Pts_Ti);
%      detJ = det(J);
%      % Reshape u to allow broadcasting
%      U1 = reshape(cu, [1, M, 1]);     % size [1 × M × 1]
%      U2 = reshape(cu, [M, 1, 1]);     % size [M × 1 × 1]
%      
%      % Contract over k,l using broadcasting
%      quad_numer = sum(U1 .* sum(dPdP .* U2, 1), 2);  % size [1 × 1 × N]
%      D = sqrt(squeeze(quad_numer));                 % size [N × 1]
%      % Numerator: Niq(i,q) = sum_k u_k * dPdP(i,k,q)
%      Niq = squeeze(sum(dPdP .* reshape(cu, [1, M, 1]), 2));  % [M×N]  
%      % Weighted basis: φ_j(x_q) * w_q
%      VW = V_abc .* (W/2.0);    % [N×M]
%      
%      % Assemble: H(i,j) = sum_q Niq(i,q)/D(q) * φ_j(x_q) * w_q
%      F = Niq ./ D';  % [M×N]
%      H = F * VW * detJ + xi * K;     % [M×M]
