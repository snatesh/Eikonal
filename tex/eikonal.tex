...
&u = q,\quad x \in \partial \Tau^d,
	\end{cases}
\end{equation}
where $f : \overline{\Tau^d} \to \bb{R}_\star^+$ is the speed in the medium, $q : \partial\Tau^d \to \bb{R}^+$ is the time to leave $\Tau^d$ from a point on $\partial\Tau^d$ ($q \equiv 0$ if no exit cost). Splitting \eqref{eq:eikonal} into homogeneous and particular problems lends itself well to numerical approximation with optimization libraries. As such, we can solve the problem in two steps. First, solve for $u_1$ in
\begin{equation}
	\begin{cases}
	&\sum_{i = 1}^d (\partial_{x_i} u_1)^2 = \big(\frac{1}{f}\big)^2,\quad x \in \Tau^d
\end{cases}
\end{equation}
...
Noting that the value of $u_1$ on the boundary is annihilated in the combined solution $$u=u_1+u_2,$$ we conclude that $u$  solves \eqref{eq:eikonal}. \color{red} WRONG: I forgot to take account the fact of nonlinearity in the PDO. What we really want is
\begin{align*}
	\big(\frac{1}{f}\big)^2 &=\sum_{i=1}^d (\partial_{x_i} u)^2 = \sum_{i=1}^d \big(\partial_{x_i} (u_1 + u_2)\big)^2 = \sum_{i=1}^d (\partial_{x_i}u_1 + \partial_{x_i}u_2)^2\\
	 &= \sum_{i=1}^d \partial_{x_i}u_1^2 + \partial_{x_i}u_2^2 + 2(\partial_{x_i}u_1)(\partial_{x_i}u_2)
\end{align*}
I missed those cross terms, which is one of the reasons I'm getting incorrect results as of now. We don't need to split the problem as such, so I will return to the original way of relying on the COBYLA algorithm for the fully constrained problem \eqref{eq:eikonal}.
...
Hence, we can frame the discrete Eikonal solver in terms of a quadratic program with equality constraints:

\begin{equation}
	\begin{split}
		&\min_{\bs{u}\in \bb{R}^n} \frac{1}{2}\bs{u}^T G \bs{u} + \bs{u}^T c\\
		&\text{subject to } A\bs{u} = b,
	\end{split}
\end{equation}
where $G = \mathcal{\tilde D}_x^T\mathcal{\tilde D}_x + \mathcal{\tilde D}_y^T \mathcal{\tilde D}_y \in \bb{R}^{N\times N}$ is symmetric, and we need to derive the form of $A$ for enforcing conditions on the boundary. \color{red} If $u$ is the time to exit, and $f$ is the speed in the medium, then there must be a way to write $f$ in terms of $u$, so that we can cast the problem completely as a QP. However, this is a non-linear relation, so I'm not sure yet how to do this with linear operations. Perhaps we proceed weakly as
\begin{equation}
	||fu||_{L^2(\Tau^2,w^{(a+1,b+1,c+1)})} = \alpha
\end{equation}
and use Parseval's theorem? Then, $||fu|| = \bs{}$ ... I'm tired and need to think/sleep/think tomorrow. But, I'm hoping that we can let $c\equiv 0$, and incorporate the speed into the constraint relation. Or, we require that the fist coefficient of $u$ is 1, i.e. that the constant polynomial has non-zero contribution to the expansion. Then, $c$ could just be $\text{1/speed} \times \bs{e}_1$, or something like that?
