...
\begin{align*}
&\int_{\Tau^2}\bs{P}^{(\bs{a+1})}(\bs{x})^T\big(\mathcal{\tilde{D}}_{x_1}\bs{u}\bs{u}^T\mathcal{\tilde{D}}_{x_1}^T + \mathcal{\tilde{D}}_{x_2}\bs{u}\bs{u}^T\mathcal{\tilde{D}}_{x_2}^T \big)w^{(\bs{a+1})}(\bs{x})\bs{P}^{(\bs{a+1})}(\bs{x})d\bs{x}\\
&\int_{\Tau^2}
\end{align*}
