...
\end{align}
and we still need to consider the constraint equation. However, by making use of a weighted basis $\bs{\tilde{P}}(\bs{x})$, and expressing our solution coefficients and differential operators in terms of this weighted basis, we can eliminate the constraint equation and solve a completely unconstrained quadratic optimization problem \color{red} This doesn't seem to work as well for the non-linear problem, but we will describe that approach and verify correctness in the next section anyway. \color{black} 
