...
We take the approach of eliminating the constraints by solving a the minimization problem on auxiliary variables. That is, suppose $\bs{P}(\bs{x}) \in \bb{R}^{M\times N}$, $M > N$, has rank $N$. We can compute the \emph{full} \textit{SVD}  of $\bs{P}(\bs{x}) =  U \Sigma V^T$. Then, the last $m-n$ rows of $V^T$ form a basis for the $\mathcal{N}(\bs{P}(\bs{x}))$. That is, $V^T = \begin{bmatrix} V_n^T \\ V_{m-n}^T \end{bmatrix}$, and 
 any solution $\bs{u}$ satisfying the equality constraints can be expressed as 
\begin{equation}\label{eq:eikonalauxvar}
\bs{u} =  V_{m-n} \bs{u}_{\text{eq}},
\end{equation}
Hence, we can solve 
\begin{align}\label{eq:eikonalqpelim}
	\min_{\bs{u}_{\text{eq}}\in \bb{R}^N}F(\bs{u}_\text{eq}) = \bs{u}_{\text{eq}}^TV_{m-n}^T G V_{m-n}\bs{u}_{\text{eq}} - \tilde{\bs{c}}_{\frac{1}{f}}^T\tilde{\bs{c}}_{\frac{1}{f}}
\end{align}
...
\begin{align}
	\nabla_{\bs{u_{\text{eq}}}}F(\bs{u_{\text{eq}}}) &= 2V_{m-n}^TGV_{m-n}\bs{u_{\text{eq}}},\\
	\nabla^2_{\bs{u}}F(\bs{u}) &= 2V_{m-n}^TGV_{m-n},
\end{align}
