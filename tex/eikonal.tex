...
We take the approach of eliminating the constraints by solving a the minimization problem on auxiliary variables. That is, suppose $\bs{P}(\bs{x}) \in \bb{R}^{M\times N}$, $M > N$, has rank $N$. We can compute the \textit{QR} decomposition of $\bs{P}(\bs{x}) =  QR = \begin{bmatrix} Q_1 & Q_2 \end{bmatrix}R$, where $Q_1 \in \bb{R}^{M\times N}$, and the orthonormal columns of $Q_2 \in \bb{R}^{M\times {(M-N)}}$ form a basis for $\text{Null}[\bs{P}(\bs{x})]$. Then, any solution $\bs{u}$ to the equality constraints can be expressed as 
\begin{equation}\label{eq:eikonalauxvar}
\bs{u} =  Q_2 \bs{u}_{\text{eq}},
\end{equation}
Hence, we can solve 
\begin{align}\label{eq:eikonalqpelim}
	\min_{\bs{u}_{\text{eq}}\in \bb{R}^N}F(\bs{u}_\text{eq}) = \bs{u}_{\text{eq}}^TQ_2^T G Q_2\bs{u}_{\text{eq}} - \tilde{\bs{c}}_{\frac{1}{f}}^T\tilde{\bs{c}}_{\frac{1}{f}}
\end{align}
...
\begin{align}
	\nabla_{\bs{u_{\text{eq}}}}F(\bs{u_{\text{eq}}}) &= 2Q_2^TGQ_2\bs{u_{\text{eq}}},\\
	\nabla^2_{\bs{u}}F(\bs{u}) &= 2Q_2^TGQ_2,
\end{align}
