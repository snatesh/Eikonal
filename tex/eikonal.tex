...
\section{Global Solutions to the Eikonal Equation}{}
...
\begin{align*}
\big(\mathcal{\tilde{D}}_{x_1}\bs{u}\bs{u}^T\mathcal{\tilde{D}}_{x_1}^T + \mathcal{\tilde{D}}_{x_2}\bs{u}\bs{u}^T\mathcal{\tilde{D}}_{x_2}^T \big)&\int_{\Tau^2}w^{(\bs{a+1})}(\bs{x})\bs{P}^{(\bs{a+1})}(\bs{x})^T\bs{P}^{(\bs{a+1})}(\bs{x})d\bs{x}\\
&=(\cdot) \int_{\Tau^2} \bs{P}^{(\bs{a+1})}\cdot\bs{P}^{(\bs{a+1})} d w^{(\bs{a+1})}(\bs{x}).
\end{align*}
