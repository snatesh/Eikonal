...
Consider orthogonal polynomials up to total degree $n-1$ in dimension $d$ defined in a convex region $\Omega \subset \mathbb{R}^d$ with boundary $\partial \Omega$. Suppose that the symmetric, real-valued Jacobi matrices $J^k \in \mathbb{R}^{\text{dim}\Pi_{n-1}^d} = \mathbb{R}^N$, $k=1,\cdots d$ corresponding to a 3-term recurrence relation on subspaces of homogoneous polynomials with total degree $m=0,\cdots n-1$ do NOT form a commuting family. Then, there exists an approximate joint eigenvalue decomposition of the matrices $J_k$ such that the $d$-tuple of matched eigenvalues $(\lambda_m^1,\cdots \lambda_m^d)\in (\Omega \setminus \partial\Omega)$ (not proven, but seems to hold numerically! We need it to hold only finite manly times, so that's ok.).
