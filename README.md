...
- Quadrature discovery with optimization parameters from `nlopt` exposed for such use. In particular, I use an interlaced scheme where first a constrained non-linear optimization problem is solved, then, if needed, a newton relaxation, followed by another non-linear problem with a different objective, and a last on-demand newton relaxation.
