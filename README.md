...
Consider orthogonal polynomials up to total degree $n-1$ in dimension $d$ defined in a convex region $\Omega \subset \mathbb{R}^d$ with boundary $\partial \Omega$. There are $\text{dim}\Pi_{n-1}^d=N$ of them. Suppose that the symmetric, real-valued Jacobi matrices $J^k \in \mathbb{R}^{N\times N}$, $k=1,\cdots d$ corresponding to a 3-term recurrence relation on subspaces of homogoneous polynomials with total degree $m=0,\cdots n-1$ do NOT form a commuting family. Then, there exists an approximate joint eigenvalue decomposition of the matrices $J_k$ such that the $d$-tuple of matched eigenvalues $(\lambda_m^1,\cdots \lambda_m^d)\in (\Omega \setminus \partial\Omega)$ (not proven, but seems to hold numerically! We need it to hold only finite manly times, so that's ok.).

That is, we claim that there exists a single unitary matrix $U$ such that the matrices 
$$D^k = U J^k U^T, \quad 1 \leq k \leq d$$
are approximately diagonal. Here, we define approximate diagonality in the sense that $D^k$ is the best representation of $J^k$, entry-wise, in the basis $U$. From a subspace fitting approach, we can cast the task of finding $U$ and $D^k$ into a quadratic optimization problem with quadratic equality constraints: 
...
The idea is to throw this into an optimization routine on vectorizations of the minimization variables, so there are $N^2(d+1)$ of them. The main question has to do with initialization:
   - Ensure the constraint is satisfied to begin with by setting $U=I$.
      - Try initializing $F^k = J^k +(\varepsilon)_{ij}$, with $(\varepsilon)_{ij}$ drawn randomly from a distribution to be determined.
